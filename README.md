...
A professional-grade implementation of dynamic memory management functions in C, featuring custom versions of `malloc`, `calloc`, `realloc`, and `free`. The project provides deep insights into low-level memory management while maintaining production-quality code standards.

## Overview

This memory allocator library demonstrates efficient virtual memory management using both heap-based (`brk`) and memory-mapped (`mmap`) allocation strategies. It implements industry-standard techniques for reducing memory fragmentation and optimizing performance.

## Features

- **Dual Allocation Strategy**: Uses `brk()` for small allocations and `mmap()` for larger blocks
- **Memory Alignment**: Ensures 8-byte alignment for optimal performance on 64-bit systems
- **Fragmentation Reduction**: Implements block splitting and coalescing algorithms
- **Heap Preallocation**: Reduces syscall overhead with strategic preallocation
- **Best-Fit Algorithm**: Optimizes block selection to minimize wasted space
...
- **Block Management**: Uses `struct block_meta` to track allocation metadata
- **Efficient Searching**: Implements best-fit block selection with O(n) complexity
- **Memory Reuse**: Recycles freed blocks to minimize system calls
- **Boundary Handling**: Properly manages block splitting and merging operations
...
This compiles the shared library `libosmem.so` that can be linked with applications.
